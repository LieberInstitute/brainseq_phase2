...
keepIndex = which(rse_gene$Age > 17 & 		# hippo only has CAUC and AA
	rse_gene$Kit == "HMR" & rse_gene$Region == "HIPPO")  
rse_gene = rse_gene[,keepIndex]
rse_exon = rse_exon[eInd,keepIndex]			# dim: 211455 333
rse_jxn = rse_jxn[jInd,keepIndex]			# dim: 238975 333
...
k = num.sv(log2(assays(cov_rse_hippo)$count + 1), mod) 		# k = 18
qSVs = pcaDeg$x[,1:k]
...
save(esftQsva, file="rdas/wgcna_soft_threshold_Hippo_exon.rda")
# load("rdas/wgcna_soft_threshold_Hippo_exon.rda")
...
enetQsva = blockwiseModules(t(exonExprsQsva), power = esftQsva$powerEstimate,
	...
	saveTOMFileBase = "rdas/Hippo_exon_qSVA")
save(enetQsva, exonMap, file = "rdas/Hippo_exon_wgcnaModules.rda")

####

# load("rdas/Hippo_exon_wgcnaModules.rda", verbose=TRUE)
...
jsftQsva = pickSoftThreshold(t(jExprsQsva), 
	powerVector = powers, verbose = 5)
save(jsftQsva, file="rdas/wgcna_soft_threshold_Hippo_jxn.rda")
# load("rdas/wgcna_soft_threshold_Hippo_jxn.rda")
...
jnetQsva = blockwiseModules(t(jExprsQsva), power = jsftQsva$powerEstimate,
	...
	numericLabels = TRUE, pamRespectsDendro = FALSE,
	saveTOMs = TRUE, 	verbose = 3, maxBlockSize = 30000,
	saveTOMFileBase = "rdas/Hippo_jxn_qSVA")
save(jnetQsva, jMap, file = "rdas/Hippo_jxn_wgcnaModules.rda")

####

# load("rdas/Hippo_jxn_wgcnaModules.rda", verbose=TRUE)
