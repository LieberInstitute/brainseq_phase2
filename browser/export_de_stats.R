...
info <-
    lapply(lapply(indv_go_cleaned_sczd, subset, padj < 0.05), function(x) {
        x[, -c(11, 12)]
    })
info <-
...
## For table S4
load(
    "/dcl01/ajaffe/data/lab/qsva_brain/brainseq_phase2_qsv/rdas/degradation_rse_phase2_usingJoint_justFirst.rda",
    verbose = TRUE
)
...
for (i in 1:4) {
    info[[i]] <-
        cbind(info[[i]], rbind(as.data.frame(rowRanges(rses[[i]])), as.data.frame(rowRanges(rses[[i]]))))
