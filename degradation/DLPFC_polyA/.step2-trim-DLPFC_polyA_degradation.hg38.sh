...
#$ -N step2-trim-DLPFC_polyA_degradation.hg38
#$ -pe local 8
#$ -o ./logs/trim-DLPFC_polyA_degradation.$TASK_ID.txt
#$ -e ./logs/trim-DLPFC_polyA_degradation.$TASK_ID.txt
#$ -t 1-20
#$ -tc 8
#$ -hold_jid pipeline_setup,step1-fastqc-DLPFC_polyA_degradation.hg38
#$ -m a
echo "**** Job starts ****"
date
...
if [ TRUE == "TRUE" ] ; then 
	...
	if [[ $RESULT1 == "FAIL" || $RESULT2 == "FAIL" ]] ; then
		...
		mkdir -p /dcl01/lieber/ajaffe/lab/brainseq_phase2/degradation/DLPFC_polyA/trimmed_fq
		...
		## rerun fastqc
		mkdir -p /dcl01/lieber/ajaffe/lab/brainseq_phase2/degradation/DLPFC_polyA/FastQC/Trimmed/${ID}
		/dcl01/lieber/ajaffe/Emily/RNAseq-pipeline/Software/FastQC_v0.11.5/fastqc 		$FP $FU $RP $RU 		--outdir=/dcl01/lieber/ajaffe/lab/brainseq_phase2/degradation/DLPFC_polyA/FastQC/Trimmed/${ID} --extract
	else
	...
	if [[ $RESULT1 == "FAIL" ]] ; then
		...
		mkdir -p /dcl01/lieber/ajaffe/lab/brainseq_phase2/degradation/DLPFC_polyA/trimmed_fq
		OUT=/dcl01/lieber/ajaffe/lab/brainseq_phase2/degradation/DLPFC_polyA/trimmed_fq/${ID}_trimmed.fastq
		...
		## rerun fastqc
		mkdir -p /dcl01/lieber/ajaffe/lab/brainseq_phase2/degradation/DLPFC_polyA/FastQC/Trimmed/${ID}
		/dcl01/lieber/ajaffe/Emily/RNAseq-pipeline/Software/FastQC_v0.11.5/fastqc $OUT 		--outdir=/dcl01/lieber/ajaffe/lab/brainseq_phase2/degradation/DLPFC_polyA/FastQC/Trimmed/${ID} --extract
	else
		echo "No trimming required!"
	fi
fi
