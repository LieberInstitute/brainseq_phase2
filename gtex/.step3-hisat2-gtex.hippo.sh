...
#$ -N step3-hisat2-gtex.hippo
#$ -pe local 8
#$ -o ./logs/hisat2-gtex.$TASK_ID.txt
#$ -e ./logs/hisat2-gtex.$TASK_ID.txt
#$ -t 1-94
#$ -tc 15
#$ -hold_jid pipeline_setup,step2-trim-gtex.hippo
#$ -m a
echo "**** Job starts ****"
date
...
# Directories
mkdir -p /dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/HISAT2_out/align_summaries
mkdir -p /dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/HISAT2_out/infer_strandness

if [ FALSE == "TRUE" ]
then
    mkdir -p /dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/HISAT2_out/unaligned
fi
...
if [ -f /dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/trimmed_fq/${ID}_trimmed_forward_paired.fastq ] ; then
	## Trimmed, paired-end
	echo "HISAT2 alignment run on trimmed paired-end reads"
	FP=/dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/trimmed_fq/${ID}_trimmed_forward_paired.fastq
	FU=/dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/trimmed_fq/${ID}_trimmed_forward_unpaired.fastq
...
###sam to bam
SAM=/dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/HISAT2_out/${ID}_hisat_out.sam
ORIGINALBAM=/dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/HISAT2_out/${ID}_accepted_hits.bam
SORTEDBAM=/dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/HISAT2_out/${ID}_accepted_hits.sorted
...
~/.local/bin/infer_experiment.py -i ${SORTEDBAM}.bam -r /dcl01/lieber/ajaffe/Emily/RNAseq-pipeline/Annotation/RSeQC/hg38.bed 1> /dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/HISAT2_out/infer_strandness/${ID}.txt 2>&1
