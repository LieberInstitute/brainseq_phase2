...
## Get the Hippocampus GTEx samples
m <- all_metadata('gtex')
m2 <- subset(m, smafrze == 'USE ME' & smts == 'Brain')
table(m2$smtsd)
hip <- subset(m2, smtsd == 'Brain - Hippocampus')
# write.table(hip$run, file = '~/Desktop/hippo_gtex_run_ids.txt', row.names = FALSE, quote = FALSE, col.names = FALSE)

## Find FASTQ files
p1 <- paste0('/dcl01/lieber/ajaffe/PublicData/SRA_GTEX/FASTQ/', hip$run, '_1.fastq.gz')
p2 <- paste0('/dcl01/lieber/ajaffe/PublicData/SRA_GTEX/FASTQ/', hip$run, '_2.fastq.gz')
