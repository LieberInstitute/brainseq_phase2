...
#$ -N step2-trim-gtex.hippo
#$ -pe local 8
#$ -o ./logs/trim-gtex.$TASK_ID.txt
#$ -e ./logs/trim-gtex.$TASK_ID.txt
#$ -t 1-94
#$ -tc 5
#$ -hold_jid pipeline_setup,step1-fastqc-gtex.hippo
#$ -m a
echo "**** Job starts ****"
date
...
if [ TRUE == "TRUE" ] ; then 
	REPORT1=/dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/FastQC/Untrimmed/${ID}/${FILEBASE1}_fastqc/summary.txt
	REPORT2=/dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/FastQC/Untrimmed/${ID}/${FILEBASE2}_fastqc/summary.txt
	...
	if [[ $RESULT1 == "FAIL" || $RESULT2 == "FAIL" ]] ; then
		...
		mkdir -p /dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/trimmed_fq
		...
		## rerun fastqc
		mkdir -p /dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/FastQC/Trimmed/${ID}
		/dcl01/lieber/ajaffe/Emily/RNAseq-pipeline/Software/FastQC_v0.11.5/fastqc 		$FP $FU $RP $RU 		--outdir=/dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/FastQC/Trimmed/${ID} --extract
	else
	...
	if [[ $RESULT1 == "FAIL" ]] ; then
		...
		mkdir -p /dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/trimmed_fq
		OUT=/dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/trimmed_fq/${ID}_trimmed.fastq
		...
		## rerun fastqc
		mkdir -p /dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/FastQC/Trimmed/${ID}
		/dcl01/lieber/ajaffe/Emily/RNAseq-pipeline/Software/FastQC_v0.11.5/fastqc $OUT 		--outdir=/dcl01/lieber/ajaffe/lab/brainseq_phase2/gtex/FastQC/Trimmed/${ID} --extract
	else
		echo "No trimming required!"
	fi
fi
