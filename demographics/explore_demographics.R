...
summpd <- function(pd) {
    data.frame(
        ...
        mapping_perc_sd = sd(mean(pd$overallMapRate) * 100),
        total_mapped = mean(mean(pd$totalMapped / 1e6)),
        total_mapped_sd = sd(mean(pd$totalMapped / 1e6)),
        chrM_mapping_perc = mean(mean(pd$mitoRate)) * 100,
        chrM_mapping_perc_sd = sd(mean(pd$mitoRate) * 100),
        assigned_gene_perc = mean(mean(pd$totalAssignedGene)) * 100,
        assigned_gene_perc_sd = sd(mean(pd$totalAssignedGene) * 100),
        rRNA_perc = mean(mean(pd$rRNA_rate)) * 100,
        rRNA_perc_sd = sd(mean(pd$rRNA_rate) * 100),
        ageonset = mean(pd$age_onset_schizo, na.rm = TRUE),
        ageonset_sd = sd(pd$age_onset_schizo, na.rm = TRUE),
        ageonset_NA_n = sum(is.na(pd$age_onset_schizo)),
        antipsychotics = mean(pd$antipsychotics, na.rm = TRUE) * 100,
        antipsychotics_sd = sd(pd$antipsychotics * 100, na.rm = TRUE),
