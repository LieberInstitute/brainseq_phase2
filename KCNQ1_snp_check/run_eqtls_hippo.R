...
table(snp[1,])/ncol(snp)	## MAF = 0.0456
...
load("../eQTL_full/eqtl_tables/rdas/pcs_hippo_4features_filtered_over13.rda")
...
save(meGene, meExon, meJxn, meTx,
	file="matrixEqtl_output_hippo_rs34097980.rda")
...
save(allEqtl, file="mergedEqtl_output_hippo_rs34097980.rda",compress=TRUE)
...
load("mergedEqtl_output_dlpfc_rs34097980.rda")
dlp = allEqtl

dlp$Region = "DLPFC"
hippo$Region = "HIPPO"

allEqtl = rbind(dlp,hippo)
allEqtl = allEqtl[order(allEqtl$pvalue, decreasing=FALSE),]

allEqtl = allEqtl[allEqtl$pvalue<0.1,]
colnames(allEqtl)[2] = "Feature"

write.csv(allEqtl, file="eqtl_output_rs34097980.csv")
