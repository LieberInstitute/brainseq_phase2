...
load(
    here::here(
        ...
        "eqtl_tables",
        "rdas",
        "pcs_hippo_4features_filtered_over13.rda"
    ),
    verbose = TRUE
)
...
save(meGene, meExon, meJxn, meTx,
	file=here::here(
        "eQTL_full",
        "subset_hancock",
        "matrixEqtl_output_hippo_4features_subset_hancock.Rdata")
)
...
allEqtl$Region <- "HIPPO"
save(allEqtl, file=here::here(
        "eQTL_full",
        "subset_hancock",
        "mergedEqtl_output_hippo_4features_subset_hancock.Rdata")
    ,compress=TRUE)
