...
keepInd = which(colData(rse_gene)$Age > 13 & colData(rse_gene)$Region == "HIPPO" & colData(rse_gene)$Race == 'CAUC')
...
save(genePCs, 
	file="rdas/pcs_hippo_gene_filtered_over13.rda")
...
save(meGene,
	file="eqtl_tables/matrixEqtl_output_hippo_gene.rda")
...
geneEqtl$gencodeTx = CharacterList(c(as.list(rowRanges(rse_gene)$gencodeTx[match(geneEqtl$gene, 
	rownames(rse_gene))])))
save(geneEqtl, file="eqtl_tables/mergedEqtl_output_hippo_gene.rda",compress=TRUE)
